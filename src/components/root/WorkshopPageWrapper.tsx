...
import {
  ...
  fetchWorkshopsPages,
} from '@/lib/actions/workshop/fetchWorkshopPagesActions';
...
const WorkshopPageWrapper = async ({
  ...
  let totalPages = 0;
  if (type === 'all') {
    totalPages = await fetchWorkshopsPages(query, user);
  } else {
    totalPages = await fetchAvailableWorkshopsPages(query, user.id!);
  }
